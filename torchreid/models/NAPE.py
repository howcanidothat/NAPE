...
def nape(num_classes, loss='part_based', pretrained=True, config=None, **kwargs):
    return NAPE(num_classes, pretrained, loss, config, **kwargs)


def pcb(num_classes, loss='part_based', pretrained=True, config=None, **kwargs):
    config.model.nape.learnable_attention_enabled = False
    return NAPE(num_classes, pretrained, loss, config, horizontal_stripes=True, **kwargs)


def bot(num_classes, loss='part_based', pretrained=True, config=None, **kwargs):
    config.model.nape.masks.parts_num = 1
    config.model.nape.learnable_attention_enabled = False
    return NAPE(num_classes, pretrained, loss, config, horizontal_stripes=True, **kwargs)
