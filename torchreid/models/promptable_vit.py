...
class ViT(nn.Module):  # TODO refactor using "PromptableTransformerBackbone"
    # This ViT implementation is obsolete and should be updated with PromptableTransformerBackbone
    """ Transformer-based Object Re-Identification
    """
    def __init__(self, config, use_negative_keypoints=False, img_size=224, patch_size=16, stride_size=16, in_chans=3, in_chans_masks=17, num_classes=1000, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0., camera=0, view=0,
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, local_feature=False, sie_xishu =1.0, masks_prompting=False, disable_inference_prompting=False, prompt_parts_num=0, **kwargs,):
        super().__init__()
        ...
        if hybrid_backbone is not None:
            ...
            self.patch_embed = PatchEmbed_overlap(
                ...
                embed_dim=embed_dim)
            self.masks_patch_embed = PatchEmbed_overlap(
                img_size=img_size, patch_size=patch_size, stride_size=stride_size, in_chans=self.in_chans_masks,
                embed_dim=embed_dim)
        ...
        self.parts_embed = nn.Parameter(torch.zeros(self.prompt_parts_num + 1, 1, embed_dim))  # +1 for background
    ...
    def forward(self, images, prompt_masks=None, keypoints_xyc=None, cam_label=None, view_label=None, **kwargs):
        images = self.patch_embed(images)  # rename to image_tokens
        images = self._mask_embed(images, prompt_masks)

        cls_tokens = self.cls_token.expand(images.shape[0], -1, -1)   # FIXME remove, use GAP
        ...
        images = self.norm(images)   # FIXME should we do this? check other methods
        images = images[:, 1:, :]\
            .transpose(2, 1)\
            .unflatten(-1, self.tkzd_img_size)  # [N, D, Hf, Wf]
        return images
...
def vit(
    ...
    **kwargs,
    ...
    model = ViT(  # FIXME all params from config
        config=config.model.promptable_trans,
        use_negative_keypoints=config.model.nape.keypoints.use_negative_keypoints,
        img_size=[config.data.height, config.data.width],
        patch_size=config.model.promptable_trans.patch_size,
        stride_size=config.model.promptable_trans.stride_size,
        embed_dim=768,
        depth=12,
        num_heads=12,
        mlp_ratio=4,
        qkv_bias=True,
        in_chans_masks=config.model.nape.masks.prompt_parts_num,
        camera=cam_num if config.model.transreid.sie_camera else 0,
        view=view if config.model.transreid.sie_view else 0,
        drop_path_rate=config.model.promptable_trans.drop_path,
        drop_rate=config.model.promptable_trans.drop_rate,
        attn_drop_rate=config.model.promptable_trans.att_drop_rate,
        ...
        prompt_parts_num=config.model.nape.masks.prompt_parts_num,
        **kwargs,
    )
    if pretrained:
        pretrained_path = os.path.join(pretrained_path, __model_filename[config.model.promptable_trans.transformer_type])
