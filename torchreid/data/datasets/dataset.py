...
class Dataset(object):
    ...
    def infer_masks_path(self, img_path, masks_dir, masks_suffix):
        masks_path = os.path.join(self.dataset_dir, self.masks_base_dir, masks_dir,
                                  os.path.basename(os.path.dirname(img_path)),
                                  os.path.splitext(os.path.basename(img_path))[0] + masks_suffix)
        return masks_path

    def infer_kp_path(self, img_path):
        masks_path = os.path.join(self.dataset_dir, 'external_annotation', self.kp_dir,
                                  os.path.basename(os.path.dirname(img_path)),
                                  os.path.splitext(os.path.basename(img_path))[0] + '.jpg_keypoints.json')
        return masks_path
    ...
    def __init__(
        ...
        self.train = train
        self.filter_out_no_skeletons = config.model.nape.keypoints.filter_out_no_skeletons
        if self.filter_out_no_skeletons:
            or_size = len(self.train)
            self.train = [sample for sample in self.train if
                          'kp_path' in sample and read_keypoints(sample['kp_path']).max() > 0]
        ...
        self.keypoints_to_prompt_masks = KeypointsToMasks(mode=self.cfg.model.nape.keypoints.prompt_masks,
                                                          vis_thresh=self.cfg.model.nape.keypoints.vis_thresh,
                                                          vis_continous=self.cfg.model.nape.keypoints.vis_continous,
                                                          )
        self.keypoints_to_target_masks = KeypointsToMasks(mode=self.cfg.model.nape.keypoints.target_masks,
                                                          vis_thresh=self.cfg.model.nape.keypoints.vis_thresh,
                                                          vis_continous=False,
                                                          )
        self.mode = mode
    ...
    #     kp_dir = self.cfg.model.nape.keypoints.kp_dir
    ...
    def combine_all(self):
        ...
        # relabel pids in gallery and query (collect all unique pids from both)
        all_pids = set()
        ...
        for sample in self.query:
            pid = sample['pid']
            if pid in self._junk_pids:
                continue
            all_pids.add(pid)

        pid2label = {pid: i for i, pid in enumerate(all_pids)}
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):  # kept for backward compatibility
        ...
        return self.getitem(sample, self.cfg, self.keypoints_to_prompt_masks, self.prompt_preprocess,
                            self.keypoints_to_target_masks, self.target_preprocess, self.transforms(self.mode),
                            self.load_masks, batch_pids, all_occluders)

    @staticmethod
    def getitem(sample, cfg, keypoints_to_prompt_masks, prompt_preprocess, keypoints_to_target_masks, target_preprocess,
                transforms, load_masks=False, batch_pids=None, all_occluders=None):
        """
        ...
        spatial_feature_shape = cfg.model.nape.spatial_feature_shape
        kp_enabled = cfg.model.nape.keypoints.enabled
        ...
        use_negative_keypoints = cfg.model.nape.keypoints.use_negative_keypoints
        masks_enabled = cfg.model.nape.masks.enabled
        # load_masks = cfg.model.nape.masks.preprocess in masks_preprocess_transforms or cfg.model.nape.masks.preprocess == 'none'
        ...
        if masks_enabled:
            if load_masks and 'masks_path' in sample:
                transf_args["masks"] = [read_masks(
                    sample['masks_path'])]  # FIXME use BasicTransform.add_targets() to create prompt masks target
            elif not load_masks:
        ...
        if kp_enabled:
            ...
            if len(keypoints) > 0:
                keypoints_xyc[kp_indices] = np.concatenate(
                    (np.array(keypoints), np.expand_dims(np.array(kp_vis_score), axis=1)), axis=1)
                ...
                if use_negative_keypoints:
                    if len(negative_keypoints_xyc) != 0:
                        ...
                        for neg_kps in negative_keypoints_xyc:
                            neg_mask = keypoints_to_prompt_masks(neg_kps, img_size,
                                                                 prompt_mask_size)  # TODO loop over skeletons and merge: needed for joints
                    ...
                    if negative_keypoints_xyc.shape[0] > max_skeletons:
                        ...
                        sample["negative_keypoints_xyc"] = np.concatenate(
                            (
                                negative_keypoints_xyc,
                                np.zeros((max_skeletons - negative_keypoints_xyc.shape[0], 17, 4))))
        ...
        if masks_enabled and 'masks' in result:
            ...
            if masks_prompting:
                if not kp_enabled or \
                    ...
                    sample["prompt_masks"] = torch.cat([masks[0][:1], masks[0][1:].max(axis=0)[0].unsqueeze(
                        0)])  # merge into segmentation mask with one channel
