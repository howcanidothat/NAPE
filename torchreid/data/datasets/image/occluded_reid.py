...
class OccludedReID(ImageDataset):
    ...
    train_dir = 'occluded_body_images'
    query_dir = 'occluded_body_images'
    gallery_dir = 'whole_body_images'
    ...
    def infer_kp_path(self, img_path):
        masks_path = os.path.join(self.dataset_dir, 'external_annotation', self.kp_dir, os.path.basename(os.path.dirname(os.path.dirname(img_path))), os.path.splitext(os.path.basename(img_path))[0] + '.tif_keypoints.json')
        return masks_path
    ...
    def __init__(self, root='', masks_dir=None, **kwargs):
        ...
        train = self.process_dir(self.train_dir, relabel=True, is_query=True)
    ...
    def process_dir(self, dir_path, relabel=False, is_query=True):
        ...
        for img_path in img_paths:
            img_name = osp.basename(img_path)  # 使用os.path.basename代替split('/')
