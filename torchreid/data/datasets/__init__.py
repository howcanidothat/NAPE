...
def _build_cache_key(name, **kwargs):
    """Build a cache key that captures parameters affecting dataset content.

    We intentionally ignore transform functions and mode so that the same
    underlying dataset object can be reused across train/query/gallery via copy.copy().
    """
    cfg = kwargs.get('config', None)

    def safe_get(attr_chain, default=None):
        cur = cfg
        try:
    ...
    kp_dir = safe_get(['model', 'nape', 'keypoints', 'kp_dir'], None)
    sample_limit = safe_get(['data', 'sample_limit'], None)
    query_limit = safe_get(['data', 'query_limit'], None)
    gallery_limit = safe_get(['data', 'gallery_limit'], None)
    keep_qg_ratio = safe_get(['data', 'keep_qg_ratio'], None)
    ...
    key = (
        ...
        ('keep_qg_ratio', keep_qg_ratio),
    )
    return key


def clear_datasets_cache():
    """Clear the in-memory datasets cache.

    Call this before (re)initializing datamanager if you changed dataset-related
    config such as limits or annotation/masks dirs.
    """
    global __datasets_cache
    __datasets_cache.clear()
    print("[torchreid.data] Datasets cache cleared.")
...
def init_image_dataset(name, mode='train', **kwargs):
    ...
    """
    key = _build_cache_key(name, **kwargs)
    if key in __datasets_cache:
        print("Using cached dataset {} with key {}.".format(name, key))
        dataset = __datasets_cache[key]
    else:
        print("Creating dataset {} with key {} and caching it.".format(name, key))
        dataset = get_image_dataset(name)(mode=mode, **kwargs)
        __datasets_cache[key] = dataset
