...
def compute_parts_num_and_names(cfg):
    mask_config = get_image_dataset(cfg.data.sources[0]).get_masks_config(
        cfg.model.nape.masks.dir if not (cfg.model.nape.keypoints.enabled and cfg.model.nape.keypoints.target_masks != 'none') else cfg.model.nape.keypoints.target_masks
    )
    if cfg.loss.name == "part_based":
        if cfg.model.nape.keypoints.enabled:
            kp_cfg = cfg.model.nape.keypoints
            masks_cfg = cfg.model.nape.masks
        ...
        if cfg.model.nape.masks.enabled:
            masks_transform = cfg.model.nape.masks.preprocess
            if (
                mask_config is not None and mask_config[1]
            ) or masks_transform == "none":
                # ISP masks or no transform
                cfg.model.nape.masks.parts_num = mask_config[0]
                cfg.model.nape.masks.parts_names = (
                    mask_config[3]
                    if 3 in mask_config
                    else [
                        "p{}".format(p)
                        for p in range(1, cfg.model.nape.masks.parts_num + 1)
                    ]
                )
                if not cfg.model.nape.keypoints.enabled or \
                        (cfg.model.nape.keypoints.enabled and cfg.model.nape.keypoints.prompt_masks == 'none'):
                    cfg.model.nape.masks.prompt_parts_num = cfg.model.nape.masks.parts_num
                    cfg.model.nape.masks.prompt_parts_names = cfg.model.nape.masks.parts_names
            else:
                if not cfg.model.nape.keypoints.enabled or \
                        (cfg.model.nape.keypoints.enabled and cfg.model.nape.keypoints.target_masks == 'none'):
                    masks_transform = masks_preprocess_all[masks_transform]()
                    cfg.model.nape.masks.parts_num = masks_transform.parts_num
                    cfg.model.nape.masks.parts_names = masks_transform.parts_names
            if cfg.model.promptable_trans.masks_prompting:
                if not cfg.model.nape.keypoints.enabled or \
                        (cfg.model.nape.keypoints.enabled and cfg.model.nape.keypoints.prompt_masks == 'none'):
                    # cfg.model.nape.masks.prompt_parts_num = cfg.model.nape.masks.parts_num
                    cfg.model.nape.masks.prompt_parts_num = 1  # FIXME only supports segmentation masks now, no multi-channel parsing labels
                    cfg.model.nape.masks.prompt_parts_names = cfg.model.nape.masks.parts_names
