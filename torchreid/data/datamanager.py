...
class DataManager(object):
    ...
    def __init__(
        ...
        self.transform_tr, self.transform_te, self.kp_target_transform, self.kp_prompt_transform = build_transforms(
            ...
            masks_preprocess=config.model.nape.masks.preprocess,
            softmax_weight=config.model.nape.masks.softmax_weight,
            background_computation_strategy=config.model.nape.masks.background_computation_strategy,
            mask_filtering_threshold=config.model.nape.masks.mask_filtering_threshold,
    ...
    def __init__(
        ...
        for name in self.sources:
            trainset_ = init_image_dataset(
                ...
                masks_dir=masks_dir,
                load_masks=self.config.model.nape.masks.preprocess in masks_preprocess_transforms or self.config.model.nape.masks.preprocess == 'none',  # none to load masks and apply no grouping
        ...
        if load_train_targets:
            ...
            for name in self.targets:
                trainset_t_ = init_image_dataset(
                    ...
                    market1501_500k=market1501_500k,
                    masks_dir=masks_dir,
                    load_masks=self.config.model.nape.masks.preprocess in masks_preprocess_transforms,
                )
