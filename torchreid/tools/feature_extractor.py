...
class NAPEFeatureExtractor(object):
    ...
    def __init__(
        ...
        _, preprocess, self.target_preprocess, self.prompt_preprocess = build_transforms(
            ...
            norm_std=pixel_std,
            masks_preprocess=cfg.model.nape.masks.preprocess,
            softmax_weight=cfg.model.nape.masks.softmax_weight,
            background_computation_strategy=cfg.model.nape.masks.background_computation_strategy,
            mask_filtering_threshold=cfg.model.nape.masks.mask_filtering_threshold,
        )

        to_pil = T.ToPILImage()

        self.keypoints_to_prompt_masks = KeypointsToMasks(mode=cfg.model.nape.keypoints.prompt_masks,
                                                          vis_thresh=cfg.model.nape.keypoints.vis_thresh,
                                                          vis_continous=cfg.model.nape.keypoints.vis_continous,
                                                          )
        self.keypoints_to_target_masks = KeypointsToMasks(mode=cfg.model.nape.keypoints.target_masks,
                                                          vis_thresh=cfg.model.nape.keypoints.vis_thresh,
                                                          vis_continous=False,
                                                          )
    ...
    def __call__(self, input):
        ...
        # Convert samples to dictionary format and create updated_samples
        updated_samples = []
        for sample in samples:
            if isinstance(sample, str):
                sample_dict = {"img_path": sample}
            else:
                sample_dict = sample
            updated_samples.append(copy.deepcopy(sample_dict))
        ...
        for i, sample in enumerate(samples):
            # Convert string path to dictionary format expected by ImageDataset.getitem
            if isinstance(sample, str):
                sample_dict = {"img_path": sample}
            else:
                sample_dict = sample
        ...
        (
            ...
            model_output, self.cfg.model.nape.test_embeddings
        )
