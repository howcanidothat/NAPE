...
import os
import numpy as np
from torchreid.tools.feature_extractor import NAPEFeatureExtractor
...
def extract_part_based_features(extractor, image_list, batch_size=400):
    ...
    for chunk in tqdm.tqdm(images_chunks):
        updated_samples, embeddings, visibility_scores, masks = extractor(chunk)
...
def extract_reid_features(cfg, base_folder, out_path, out_figure_path, model=None, model_path=None, num_classes=None):
    extractor = NAPEFeatureExtractor(
        cfg,
        model=model,
        verbose=False
    )
    ...
    for folder in folder_list:
        image_list = glob.glob(os.path.join(folder, "**/*.tif"), recursive=True)
        image_list.sort(key=extract_det_idx)
        
        # 只处理前200张图片作为测试样例
        image_list = image_list[:200]
        print("{} images to process for folder {} (limited to first 200 for testing)".format(len(image_list), folder))
        results = extract_part_based_features(extractor, image_list, batch_size=8)  # 大幅减少批处理大小
