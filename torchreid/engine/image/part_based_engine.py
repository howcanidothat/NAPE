...
class ImagePartBasedEngine(Engine):
    ...
    def __init__(
        ...
        self.parts_num = self.config.model.nape.masks.parts_num
    ...
    def _feature_extraction(self, data_loader):
        ...
        for batch_idx, data in enumerate(tqdm(data_loader, desc=f"Batches processed")):
            ...
            (
                ...
                model_output, self.config.model.nape.test_embeddings
            )
    ...
    def _evaluate(
        ...
        try:
            cfg_nape = self.config.model.nape
            if cfg_nape.mask_filtering_testing and (qf_parts_visibility is not None) and (
        ...
        if getattr(self.config.model.nape, "empty_visibility_fallback", True):
        ...
        tau = float(getattr(self.config.model.nape, "kp_quality_threshold", 0.20))
    ...
    def display_individual_parts_ranking_performances(
        ...
        bp_offset = 0
        if GLOBAL in self.config.model.nape.test_embeddings:
            bp_offset += 1
        if FOREGROUND in self.config.model.nape.test_embeddings:
            bp_offset += 1
        table = []
        for bp in range(
    ...
    def parse_data_for_train(self, data):
        ...
        if target_masks is not None:
            assert target_masks.shape[1] == (
                self.config.model.nape.masks.parts_num + 1
            ), f"masks.shape[1] ({target_masks.shape[1]}) != parts_num ({self.config.model.nape.masks.parts_num + 1})"
