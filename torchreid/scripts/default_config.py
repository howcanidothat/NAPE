...
def get_default_config():
    ...
    # project
    cfg.project = CN()
    cfg.project.name = "NAPE"  # will be used as WanDB project name
    ...
    cfg.model.nape = CN()
    cfg.model.nape.spatial_feature_shape = []  # do not change
    cfg.model.nape.pooling = "gwap"  # ['gap', 'gmp', 'gwap', 'gwap2']
    cfg.model.nape.normalization = (
        "identity"  # ['identity', 'batch_norm_2d'] - obsolete, always use identity
    )
    cfg.model.nape.mask_filtering_training = False  # use visibility scores at training - do not have an influence on testing performance yet, to be improved
    cfg.model.nape.mask_filtering_testing = True  # use visibility scores at testing - do have a big influence on testing performance when activated
    cfg.model.nape.last_stride = (
        1  # last stride of the resnet backbone - 1 for better performance
    )
    cfg.model.nape.dim_reduce = "after_pooling"  #  where to apply feature dimensionality reduction (before or after global pooling) ['none', 'before_pooling', 'after_pooling', 'before_and_after_pooling', 'after_pooling_with_dropout']
    cfg.model.nape.dim_reduce_output = (
        512  # reduce feature dimension to this value when above config is not 'none'
    )
    cfg.model.nape.backbone = (
    ...
    cfg.model.nape.learnable_attention_enabled = True  # use learnable attention mechanism to pool part features, otherwise, use fixed attention weights from external (pifpaf) heatmaps/masks
    cfg.model.nape.test_embeddings = [
        "bn_foreg",
        "parts",
    ]  # embeddings to use at inference among ['globl', 'foreg', 'backg', 'conct', 'parts']: append 'bn_' suffix to use batch normed embeddings
    cfg.model.nape.test_use_target_segmentation = "none"  # ['soft', 'hard', 'none'] - use external part mask to further refine the attention weights at inference
    cfg.model.nape.training_binary_visibility_score = True  # use binary visibility score (0 or 1) instead of continuous visibility score (0 to 1) at training
    cfg.model.nape.testing_binary_visibility_score = True  # use binary visibility score (0 or 1) instead of continuous visibility score (0 to 1) at testing
    cfg.model.nape.kp_quality_threshold = 0.20
    cfg.model.nape.visibility_binary_threshold = 0.5  # threshold for binarizing visibility scores at test time
    cfg.model.nape.empty_visibility_fallback = True  # if a sample has no visible parts (all-zero visibility), use an all-visible mask as fallback at test time
    cfg.model.nape.use_prompt_visibility_score = False  # use visibility score derived from the prompt instead of learned attention maps
    cfg.model.nape.enable_fpn = False  # use feature pyramid network (FPN)
    cfg.model.nape.fpn_out_dim = 1024  # TODO fpn and msf # internal channel size for FPN
    cfg.model.nape.enable_msf = True  # fuse multi stage feature maps to build high resolution feature maps from all stages of backbone. If disabled, spatial features of last stage are used
    cfg.model.nape.msf_spatial_scale = -1  # spatial scale of the FPN: output feature map size = input image size / msf_spatial_scale. If set to -1, output feature map size = feature map size of backbone first stage
    cfg.model.nape.shared_parts_id_classifier = False  # if each part branch uses share weights for the identity classifier. Used only when the identity loss is used on part-based embeddings.

    # Configs for the keypoints prompts given as input to NAPE
    cfg.model.nape.keypoints = CN()
    cfg.model.nape.keypoints.enabled = True  # enable keypoint prompts in dataloader (load keypoints from disk, transforms and send to training engine)
    cfg.model.nape.keypoints.vis_thresh = 0.3  # don't use keypoints with visibility score equal or below this threshold
    cfg.model.nape.keypoints.vis_continous = False  # heatmaps derived from prompt also takes into account the visibility score for the gaussian peak value (only for prompts)
    cfg.model.nape.keypoints.prompt_masks = "keypoints_gaussian"  # heuristic to generate prompt mask (heatmaps) from keypoints, {"keypoints", "keypoints_gaussian", "joints", "joints_gaussian"}
    cfg.model.nape.keypoints.prompt_preprocess = "cck6"  # parts grouping strategy for prompt masks
    cfg.model.nape.keypoints.target_masks = "none"  # heuristic to generate target human parsing labels from keypoints. Can be used if no PifPaf human parsing labels available{"keypoints", "keypoints_gaussian", "joints", "joints_gaussian"}
    cfg.model.nape.keypoints.target_preprocess = "none"  # parts grouping strategy for target masks
    cfg.model.nape.keypoints.use_negative_keypoints = True  # use keypoints from other non targets persons as negative prompts
    cfg.model.nape.keypoints.kp_dir = "pifpaf_keypoints_pifpaf_maskrcnn_filtering"  # subdirectory where keypoints are stored
    cfg.model.nape.keypoints.filter_out_no_skeletons = False  # remove training samples with no keypoints (+-203 for occluded duke)

    # Configs for the masks (target human parsing labels) used to train the body part attention head
    cfg.model.nape.masks = CN()
    cfg.model.nape.masks.enabled = True  # use masks loaded from disk or generated from heuristics like horizontal stripes
    cfg.model.nape.masks.type = "disk"  # when 'disk' is used, load part masks from storage in 'cfg.model.nape.masks.dir' folder  # TODO remove
    # when 'stripes' is used, divide the image in 'cfg.model.nape.masks.parts_num' horizontal stripes in a PCB style.
    # 'stripes' with parts_num=1 can be used to emulate the global method Bag of Tricks (BoT)
    cfg.model.nape.masks.parts_num = 1  # number of part-based embedding to extract. When PCB is used, change this parameter to the number of stripes required
    cfg.model.nape.masks.parts_names = ["1"]   # do not change
    cfg.model.nape.masks.prompt_parts_num = 1  # number of part-based embedding to extract. When PCB is used, change this parameter to the number of stripes required
    cfg.model.nape.masks.prompt_parts_names = ["1"]  # do not change
    cfg.model.nape.masks.dir = "pifpaf_maskrcnn_filtering"  # masks will be loaded from 'dataset_path/masks/<cfg.model.nape.masks.dir>' directory
    cfg.model.nape.masks.preprocess = "five_v"  # how to group the 36 pifpaf parts into smaller human semantic groups ['eight', 'five', 'four', 'two', ...], more combination available inside 'torchreid/data/masks_transforms/__init__.masks_preprocess_pifpaf'
    cfg.model.nape.masks.softmax_weight = 15
    cfg.model.nape.masks.background_computation_strategy = (
        "threshold"  # threshold, diff_from_max
    )
    cfg.model.nape.masks.mask_filtering_threshold = 0.5

    # configs for transformers-based promptable backbones, affecting NAPE when a transformer is used as backbone
    ...
    cfg.model.promptable_trans.masks_prompting = True  # disable prompting mechanism, i.e. neither pass keypoint, nor seg masks prompts to NAPE
    ...
    # configs from TransReID, affecting NAPE when a transformer is used as backbone
    ...
    # configs from SOLIDER, affecting NAPE when SOLIDER is used as backbone
    ...
    cfg.data.sample_limit = 0  # 0 or <=0: no limit; >0: limit number of images per split (train/query/gallery)
    cfg.data.query_limit = 0  # 0 = no limit; >0 limit number of query images
    cfg.data.gallery_limit = 0  # 0 = no limit (or auto if keep_qg_ratio=True); >0 limit number of gallery images
    cfg.data.keep_qg_ratio = False  # when True and only query_limit is set, gallery_limit is computed by original gallery:query ratio
    ...
    cfg.data.limit_test_samples = 0  # limit number of test samples (0 means no limit)
    cfg.data.mot = (
        CN()
    ...
    cfg.loss = CN()
    cfg.loss.name = (
        "part_based"  # use part based engine to train nape with GiLt loss
    )
    ...
    cfg.loss.part_based.weights[GLOBAL].tr = 1.0
    ...
    cfg.loss.part_based.weights[PARTS].id = 0.5
    ...
    cfg.loss.lambda_pp = 0.5
    cfg.loss.lambda_cons = 0.3
    cfg.loss.part_based.weights.pixels = CN(new_allowed=True)

    # test
    cfg.test = CN()
    cfg.test.batch_size = 8  # 或者更小，如4
    cfg.test.batch_size_pairwise_dist_matrix = 100  # 默认是500
    ...
    cfg.test.visrank = False  # 禁用可视化排名功能，避免出现'gallery_filter'错误
    ...
    cfg.test.prompt_fuse_alpha = 0.8
...
keys_to_ignore_in_diff = {
    ...
    "cfg.model.nape.backbone",
    "cfg.model.nape.learnable_attention_enabled",
    "cfg.model.nape.masks.parts_num",
    "cfg.model.nape.masks.dir",
    "cfg.data.type",
    "cfg.data.root",
    "cfg.data.sources",
    "cfg.data.targets",
...
def imagedata_kwargs(cfg):
    return {
        ...
        "masks_dir": cfg.model.nape.masks.dir,
    }
