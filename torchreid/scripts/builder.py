    ...
    if cfg.data.type == "image":
        if cfg.loss.name == "softmax":
            ...
            engine = torchreid.engine.ImagePartBasedEngine(
                ...
                mask_filtering_training=cfg.model.nape.mask_filtering_training,
                mask_filtering_testing=cfg.model.nape.mask_filtering_testing,
            )
...
def build_config(args=None, config=None, config_path=None, display_diff=False):
    ...
    if (
        ...
        if "config" in checkpoint:
            ...
            nape_config = checkpoint["config"].model.nape
            ...
            nape_config.pop("backbone_pretrained_path", None)
            nape_config.masks.pop("dir", None)
            if "keypoints" in nape_config:
                nape_config.keypoints.pop("enabled", None)
                nape_config.keypoints.pop("kp_dir", None)
            if cfg.model.discard_test_params:
                nape_config.masks.pop("dir", None)
                nape_config.masks.pop("preprocess", None)
                nape_config.pop("mask_filtering_testing", None)
                nape_config.pop("learnable_attention_enabled", None)
                nape_config.pop("test_embeddings", None)
                nape_config.pop("test_use_target_segmentation", None)
                nape_config.pop("testing_binary_visibility_score", None)
            cfg.model.nape.merge_from_other_cfg(nape_config)
...
def build_torchreid_model_engine(cfg):
    ...
    writer.init_engine_state(engine_state, cfg.model.nape.masks.parts_num)
    print("Building model: {}".format(cfg.model.name))
    # 确保num_train_pids至少为200，避免num_classes错误，因为Occluded_REID有200个人
    num_train_pids = max(datamanager.num_train_pids, 200)
    model = build_model(cfg, num_train_pids, datamanager.train_loader.dataset.cam_num, datamanager.train_loader.dataset.view)
...
def build_model(cfg, num_train_pids=1, cam_num=0, view=0):
    ...
    cfg.model.nape.spatial_feature_shape = model.spatial_feature_shape
